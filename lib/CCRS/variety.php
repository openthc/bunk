...
$col_list = 'Strain,StrainType,CreatedBy,CreatedDate';
...
foreach ($src_data as $idx => $rec) {
	...
	if ( ! preg_match('/^\w.+$/', $rec[0])) {
		_exit_text(sprintf('Record %d; Invalid Variety Name', $idx), 400);
	}
	if ( ! preg_match('/^(Hybrid|Indica|Sativa)$/', $rec[1])) {
		_exit_text(sprintf('Record %d; Invalid Variety Type', $idx), 400);
	}
...
$out_info[] = CCRS::assertFileName('/^strain_(\d{6}|\w{10})_(\d{14})\.csv$/');
$out_text = implode("\n", $out_info);
_exit_text($out_text);
